...
from models import User, Game, GameStatus, GameUser, Move, Rating
...
class Database:
    ...
    def create_move(self, game_id: int, user_id: str, col: int, row: int, sign: str):
        try:
            cursor = self.database.cursor()
            query = "INSERT INTO moves (game_id, user_id, col, row, sign) VALUES (?, ?, ?, ?, ?) RETURNING *"
            args = (game_id, user_id, col, row, sign)
            cursor.execute(query, args)
    ...
    def get_moves(self, game_id: int) -> list[Move] | None:
        try:
            ...
            rows = cursor.fetchall()
            moves = [Move(*move) for move in rows]
            return moves
        except Exception:
            print(traceback.format_exc(), file=sys.stderr)
            return None
    ...
    def get_rating(self) -> list[Rating] | None:
        try:
            cursor = self.database.cursor()
            query = "SELECT users.username, COUNT(wins.user_id) FROM users JOIN wins ON wins.user_id = users.id GROUP BY users.username ORDER BY COUNT(wins.user_id) DESC"
            cursor.execute(query)
            rows = cursor.fetchall()
            rating = [Rating(*rating) for rating in rows]
            return rating
        except Exception:
            print(traceback.format_exc(), file=sys.stderr)
            return None
